...
qc_sessions = Table(
    ...
    Column(
        ...
        JSONB,
        nullable=False,
        server_default=text("'{\"replicas\": 1, \"agreement_threshold\": 0.66, \"max_votes\": 3}'::jsonb"),
...
qc_verdicts = Table(
    ...
    metadata,
    Column("id", UUID(as_uuid=True), primary_key=True, server_default=text("gen_random_uuid()")),
    Column("session_id", UUID(as_uuid=True), ForeignKey("qc_sessions.id", ondelete="CASCADE"), nullable=False),
    Column(
        "item_key",
        String(1024),
        nullable=False,
    ...
    Column("image_id", UUID(as_uuid=True), ForeignKey("images.id", ondelete="CASCADE"), nullable=True),
    Column("reviewer_id", UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False),
    Column("verdict", String(20), nullable=False, comment="good, refine or bad"),
    Column("tag", String(255), nullable=True, comment="Free-form grouping tag carried from the QC tool"),
    Column(
        "corrected_label_id",
        UUID(as_uuid=True),
...
qc_consolidated = Table(
    ...
    Column("votes", JSONB, nullable=False, server_default=text("'{}'::jsonb"), comment="reviewer_id -> verdict"),
    Column("updated_at", DateTime(timezone=True), nullable=False, server_default=text("now()")),

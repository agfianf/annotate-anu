"""Repository layer for SharedImageTag (junction) data access."""
...
class SharedImageTagRepository:
    """Async repository for shared image tag operations."""
    ...
    async def add_tag(
        ...
        """Add a tag to an image. Returns None if already exists."""
        # Check if already exists
        existing = await SharedImageTagRepository.get_link(connection, shared_image_id, tag_id)
        if existing:
            return existing
    ...
    async def get_link(
        ...
        """Get the link between image and tag."""
    ...
    async def get_tags_for_image(
        ...
        stmt = (
            ...
            .where(shared_image_tags.c.shared_image_id == shared_image_id)
    ...
    async def bulk_add_tags(
        ...
        """Bulk add tags to multiple images. Returns count of new links created."""
        count = 0
        for image_id in shared_image_ids:
            for tag_id in tag_ids:
                result = await SharedImageTagRepository.add_tag(
                    connection, image_id, tag_id, user_id
                )
                if result:
                    count += 1
        return count
    ...
    async def bulk_remove_tags(
        ...
        """Bulk remove tags from multiple images. Returns count of links removed."""
        count = 0
        for image_id in shared_image_ids:
            for tag_id in tag_ids:
                removed = await SharedImageTagRepository.remove_tag(
                    connection, image_id, tag_id
                )
                if removed:
                    count += 1
        return count

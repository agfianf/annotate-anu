...
This document describes the workflow for exporting datasets from Annotate ANU.

## Overview

Annotate ANU currently has two “export” paths:

1) **Project exports (Team Mode / API-core)**: filter-aware, versioned exports created from the **Project → Explore** tab via the **Export Wizard**. These run on the backend (Celery) and appear in **Export History** with `resolved_metadata` + diff tools.
2) **Quick export (Annotation workspace)**: an in-browser export modal that downloads **COCO/YOLO** from the currently loaded `images/annotations/labels`. This is not versioned and does not use `resolved_metadata`.

This doc focuses on **Project exports (Team Mode / API-core)** unless a section explicitly says otherwise.

Key features:
- **Filter-aware**: export a subset of your dataset based on Explore filters
- **Self-documenting COCO**: backend COCO JSON embeds `info.export_config`
- **Point-in-time metadata**: `resolved_metadata` captures tags/labels/user/project as of export time
- **Version diff (high-level)**: compare filters + counts between export versions
...
sequenceDiagram
    ...
    FE->>API: GET /api/v1/projects/{project_id}/exports/classification-options
    ...
    FE->>API: POST /api/v1/projects/{project_id}/exports
    ...
    loop Poll every 2s
        FE->>API: GET /api/v1/projects/{project_id}/exports/{id}
    ...
    FE->>API: GET /api/v1/projects/{project_id}/exports/{id}/download
...
- **CSV Manifest (`manifest_csv`)**: A single `manifest.csv` inside the export ZIP.

**Output formats (planned)**:
- **ImageFolder (`image_folder`)** exists in the API schema/types, but is not generated by the current backend export task/ZIP builder yet.
...
### Export ZIP Layout (Team Mode)

Current backend exports always produce a ZIP at `settings.EXPORT_ROOT` (default: `/data/exports`).

- `annotations.json` (when `output_format=coco_json`)
- `manifest.csv` (when `output_format=manifest_csv`)
- `metadata.json` (small file with export id + timestamp + format)
- `images/**` (only when `include_images=true`)

> Note: image bytes are copied from `settings.SHARE_ROOT` (default: `/data/share`) using each image’s `file_path`.
...
```csv
file_path,class_name,split
images/cat001.jpg,cat,train
images/dog002.jpg,dog,val
images/cat003.jpg,cat,test
```

The `file_path` values are paths relative to `settings.SHARE_ROOT` (for Team Mode exports).

### Quick Export (Annotation Workspace)

The annotation workspace export modal generates COCO/YOLO directly in the browser (no versioning, no `resolved_metadata`, no Celery). See `apps/web/src/components/ExportModal.tsx`.
...
5. **Name/describe exports**: Use the export description to capture intent ("train set v3, QA'd, no blurry")
...
With `resolved_metadata` stored in the database (including tag category info), the UI can diff exports.
...
The current diff is a **high-level dataset diff**:
- Filter changes: included/excluded tags, label filter, AND/OR match modes
- Summary changes: `image_count`, `annotation_count`, `class_counts`, `split_counts`

It is **not** an annotation-content diff (it does not compare polygon edits, bbox coordinates, per-image label changes, etc.). For ideas on making diffs more useful for annotation lifecycle, see `docs/plan_exporter.md`.

## Technical Details

### Storage Location
Exports are stored at: `{EXPORT_ROOT}/{project_id}/{export_id}/export_{export_id}.zip` (defaults to `/data/exports/...`).
...
| `/api/v1/projects/{id}/saved-filters/{filter_id}` | PATCH | Update filter |
